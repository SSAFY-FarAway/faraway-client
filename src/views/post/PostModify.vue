      ...
      <div v-for="attachment in post.attachments" :key="attachment.id">
        <span class="" :id="attachment.id" v-if="check(attachment.id)">
          ...
          <b-icon
            ...
            @click="addDeleteId(attachment.id)"
          ></b-icon>
        </span>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    modify() {
      ...
      http
        .put(`/post/${this.$route.params.id}`, formData, {
          headers: {
        ...
        .then((response) => {
          ...
          if (response.status === 200) {
            ...
            this.$router.replace(
              `/post/${this.$route.params.categoryId}/${response.data}`
            );
          }
        });
    },
    ...
    getPost() {
      http.get(`/post/${this.$route.params.id}`).then((response) => {
        console.log(response);
