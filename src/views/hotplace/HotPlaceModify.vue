  ...
  <div class="hotplace-container">
    ...
    <form class="text-center mt-4" name="myform" id="myform">
      <fieldset>
        ...
        <span id="ratingview" class="px-3">5.0 / {{ hotPlace.rating }}.0 </span>
    ...
    <div class="row p-0 m-0 justify-content-end">
      ...
      <button class="btn btn-primary ml-2" @click="modify">피드 수정</button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    modify() {
      ...
      formData.append(
        "request",
        new Blob([JSON.stringify(this.hotPlace)], { type: "application/json" })
      );
      http
        .put(`/hot-place/${this.$route.params.id}`, formData, {
          headers: {
        ...
        .then((response) => {
          ...
          if (response.status === 200) {
            this.$alertSuccess("피드 수정 완료", "해당 피드로 이동합니다.");
            this.$router.replace(`/hot-place/${response.data}`);
          }
        })
        .catch(() => {
          this.$alertDanger("피드 수정 실패", "잠시 후 다시 시도하세요.");
        });
    },
    findZipCode() {
      new window.daum.Postcode({
        oncomplete: (data) => {
          ...
          this.hotPlace.zipcode = data.zonecode; //5자리 새우편번호 사용
          this.hotPlace.mainAddress = fullRoadAddr;
        },
      }).open();
    },
    ratingClick(event) {
      this.hotPlace.rating = event.target.value;
    },
    addDeleteId(id) {
